...
from application.app_handlers import select_file_path, video_redecoding
...
class MarkWindow(tk.Toplevel):
    def __init__(self, parent, config, video_path):
        ...
        temp_video_path = video_redecoding(self.video_path, self.data_folder/'temp_videos')
        # load_window.destroy()
        self.config(cursor="")
    ...
    def create_widgets(self):
        ...
        self.cb_exercise.grid(column=0, row=1, sticky=tk.NW, padx=(10,), pady=(0,10))

        self.exercise_begin.grid(column=1, row=0, sticky=tk.SW)
        self.exercise_end.grid(column=1, row=1, sticky=tk.NW)
        ...
        self.end_time.grid(column=1, row=0, sticky=tk.W)
        self.save_button.grid(column=0, row=1, columnspan=2, sticky=tk.W)
